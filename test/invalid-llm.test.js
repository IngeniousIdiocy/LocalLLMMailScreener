...
test('marks processed and records error on invalid JSON from LLM', async () => {
  const emails = buildEmails(['bad1']);
  const mockGmail = createMockGmail(emails);
  const llmStub = makeLLMStub(fixtures.llm.invalid);
  ...
  assert.ok(state.processed.bad1);
  assert.strictEqual(state.processed.bad1.status, 'error');
